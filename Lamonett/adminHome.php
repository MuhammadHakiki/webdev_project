<?php
    ...
    if(isset($_POST['upload'])) {
        ...
        $size = $_POST['ukuran'];
        $ter = "";
        $uploadFolder1 = 'images/';
        $imageTmpName1 = $_FILES['uploadfile1']['tmp_name'];
        $imageName1 = $_FILES['uploadfile1']['name'];
        if($result = move_uploaded_file($imageTmpName1,$uploadFolder1.$imageName1)) {
            $message = "success";
            }else {
            $message = "failed";
            }
        $sql = "INSERT INTO pijamas (nama, deskripsi, harga, ukuran, gambar, tersedia) VALUES 
        ('$name', '$description', '$price', '$size', '$imageName1', '$ter')";
        $stmt = $pdo->prepare($sql);

        if ($stmt->execute(['nama' => $name, 'deskripsi' => $description, 
        'harga' => $price, 'ukuran' => $size, 'gambar' => $imageName1])) {

        }
    }
    $fetch = "SELECT * FROM bottom";
    ...
    $(document).ready(function(){
      $(window).scroll(function(){
          ...
          if (scroll > 400) {
            ...
            $(".icon-bar").css("background-color", "black", "!important")
            ...
            $(".icon-bar").css("background-color", "white", "!important")
...
<body>
  <div id="header" class="navbar smart-scroll navbar-fixed-top">
    <div class="container-fluid">
      <div class="navbar-header">
        ...
        <a class="nav navbar-nav navbar-center" id="imgs3" href="#">
  ...
  <form action="" method="post" enctype="multipart/form-data">
    ...
    <center>
      ...
      <input type="text" class="form-control" id="email" placeholder="Product Name" name="nama" required=
                            "required" value="<?php 
    ...
    <br>
    <div class="form-group">
    <label for="exampleFormControlTextarea1">Producr Description</label>
    <textarea class="form-control" name="deskripsi" id="exampleFormControlTextarea1" placeholder="Product Description" rows="7" required=
                            "required" value="<?php 
                            if($_SERVER['REQUEST_METHOD'] == 'POST') 
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control" id="pwd" placeholder="Product Price" name="harga" required=
                            "required" value="<?php 
                            if($_SERVER['REQUEST_METHOD'] == 'POST') 
                            echo $price ?>">
    </div>
    <div class="form-group">
      <label for="pwd">Product Size:</label>
      <input type="text" class="form-control" id="pwd" placeholder="Product Price" name="ukuran" required=
                            "required" value="<?php 
                            if($_SERVER['REQUEST_METHOD'] == 'POST') 
                            echo $size ?>">
    </div>
        ...
        <div class="col-sm-4">
            <div class="product-grid">
                ...
                <div class="product-content">
                    <h3 class="title"><a href="#"> <?php echo $names ?></a></h3>
                    <div class="price"> <?php echo $harga ?>
                    </div>
                </div>
            </div>
        </div>
